...
export default function Sidebar() {
  ...
  return <aside className="sidebar">
    ...
    <nav>{links.filter(link => canAccess(role, link.to)).map(({to, label, icon: Icon}) =>
      <NavLink key={to} to={to} className={({isActive}) => isActive ? 'nav-link active' : 'nav-link'}><Icon size={17}/>{label}{['/tokens'].includes(to) &&<small className="mock-pill">MOCK</small>}</NavLink>
    )}</nav>
