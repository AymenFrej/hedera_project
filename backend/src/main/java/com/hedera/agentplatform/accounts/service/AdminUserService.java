...
public class AdminUserService {
    ...
    @Transactional public ManagedUserResponse updateRole(String auth,String id,RoleUpdateRequest request){ UserEntity actor=requireAdmin(auth); if(actor.id.equals(id)) throw new IllegalArgumentException("You cannot change your own role"); UserEntity user=users.findById(id).orElseThrow(); if("DISABLED".equals(user.role)) throw new IllegalArgumentException("Account is closed"); user.role=normalizeRole(request==null?null:request.role()); sessions.invalidateUser(user); return map(users.save(user)); }
    @Transactional public void delete(String auth,String id){ UserEntity actor=requireAdmin(auth); if(actor.id.equals(id)) throw new IllegalArgumentException("An administrator cannot delete their own account"); UserEntity user=users.findById(id).orElseThrow(); sessions.invalidateUser(user); AccountEntity account=accounts.findById(user.accountId).orElseThrow(); account.status="CLOSED"; accounts.save(account); user.role="DISABLED"; users.save(user); }
